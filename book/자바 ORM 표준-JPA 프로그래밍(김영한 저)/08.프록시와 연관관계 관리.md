...
> ⚠️ 주의사항 : 수년전부터 이어져온 버그때문에 `CascadeType.PERSIST`와 같이 사용해야 동작한다. 그렇지 않으면 DELETE 쿼리가 나가지 않는다.
...
하지만 `orphanRemoval = true`는 부모와 자식의 연관관계가 끊어지면(자식의 FK=null), 자식이 자동으로 삭제되는 것이다.
