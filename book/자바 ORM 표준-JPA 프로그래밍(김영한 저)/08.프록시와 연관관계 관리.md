...
![500](/assets/why-tomany-default-lazy.png)
...
`LG 트윈스` 소속 `문성훈`이 있을때, 부모인 문성훈을 삭제하면, 자식 `LG 트윈스`까지 삭제가 전파된다.
#### CascadeType.MERGE
...
고아 관리는 고아가 된 객체를 어떻게 관리할거냐?? 를 의미한다.

`@ManyTo~`(N:) 관계에선 설정할 수 없다. 왜냐면 고아객체를 가지고 있던게 `Many`니깐, 다른 데이터에서 자식을 참조하고 있다면 문제가 될 수 있기 때문이다.
